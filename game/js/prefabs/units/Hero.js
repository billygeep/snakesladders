...
var HeroGraphic = function (game_state, _x, _y) {
  // call Phaser.Sprite constructor
  Phaser.Sprite.call(this, game_state, _x, _y, 'lincoln_sprites', 'lincolnleft/0001');
  ...
  this.getAnimationName = function () {
    ...
    var scale = gameview.hero.position.y/500;
    this.scale.y = scale;
    this.scale.x = Math.abs(scale*gameview.hero.scale.x);

    if (globalvars.bosslevel) {
        name = 'crawlstop'; 
    } else {
      name = 'stop';
    }
    ...
    if (gameview.eyes) {
      name = 'stopdark';
      if (gameview.leftKey.isDown || gameview.rightKey.isDown || gameview.upKey.isDown || gameview.downKey.isDown || gameview.touchpaddown[0] === 1 || gameview.touchpaddown[1] === 1 || gameview.touchpaddown[2] === 1 || gameview.touchpaddown[3] === 1) {
        name = 'eyes';
      }
    } else if (gameview.specialstance === 1) {
      name = 'back';
    } else if (gameview.specialstance === 2) {
      name = 'stairleft';
    } else if (gameview.specialstance === 3) {
      name = 'lincolnexit';
    } else if (gameview.specialstance === 4) {
      name = 'stairright';
    } else {
      if (gameview.hero.scale.x === -1 && gameview.leftKey.isDown || gameview.touchpaddown[0] === 1 || gameview.touchpaddown[1] === 1) {
        if (globalvars.bosslevel) {
          name = 'crawl'; 
          this.scale.x *= -1
        } else {
      ...
      } else if (gameview.hero.scale.x === 1 && gameview.rightKey.isDown || gameview.upKey.isDown || gameview.touchpaddown[2] === 1 || gameview.touchpaddown[3] === 1) {
        if (globalvars.bosslevel) {
          name = 'crawl'; 
        } else {
          name = 'walkright';
        }
      } else if (gameview.downKey.isDown) {
        if (globalvars.bosslevel) {
          name = 'crawlfront'; 
        } else {
          name = 'walkright';
...
HeroGraphic.prototype = Object.create(Phaser.Sprite.prototype);
HeroGraphic.prototype.constructor = HeroGraphic;
